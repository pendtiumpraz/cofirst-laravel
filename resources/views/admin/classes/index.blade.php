                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($classes as $class)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ ucfirst($class->type) }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $class->enrollments_count }} / {{ $class->max_students }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $class->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                                {{ $class->status }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            ...
                                            <form action="{{ route('admin.classes.destroy', $class) }}" method="POST" class="inline-block ml-4">
                                                @csrf
                                                @method('DELETE')
