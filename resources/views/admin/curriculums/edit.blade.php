    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Edit Kurikulum') }}: {{ $curriculum->name }}
            </h2>
            <a href="{{ route('admin.curriculums.show', $curriculum) }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                {{ __('Kembali') }}
            </a>
        </div>
    </x-slot>
                    ...
                    <form action="{{ route('admin.curriculums.update', $curriculum) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name', $curriculum->name) }}" 
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="4" 
                                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                          placeholder="Masukkan deskripsi kurikulum...">{{ old('description', $curriculum->description) }}</textarea>
                                @error('description')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <select name="status" id="status" 
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    <option value="active" {{ old('status', $curriculum->status) == 'active' ? 'selected' : '' }}>Aktif</option>
                                    <option value="inactive" {{ old('status', $curriculum->status) == 'inactive' ? 'selected' : '' }}>Tidak Aktif</option>
                                </select>
                                @error('status')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <textarea name="learning_objectives" id="learning_objectives" rows="3" 
                                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                          placeholder="Masukkan tujuan pembelajaran...">{{ old('learning_objectives', $curriculum->learning_objectives) }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea name="prerequisites" id="prerequisites" rows="3" 
                                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                          placeholder="Masukkan prasyarat...">{{ old('prerequisites', $curriculum->prerequisites) }}</textarea>
                                @error('prerequisites')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" name="duration_weeks" id="duration_weeks" 
                                           value="{{ old('duration_weeks', $curriculum->duration_weeks) }}" 
                                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                                           min="1">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="total_hours" id="total_hours" 
                                           value="{{ old('total_hours', $curriculum->total_hours) }}" 
                                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                                           min="1" step="0.5">
                                    @error('total_hours')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="flex items-center justify-end mt-6 space-x-3">
                            <a href="{{ route('admin.curriculums.show', $curriculum) }}" 
