<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Progress {{ $child->name }} - {{ $class->name }}
            </h2>
            <a href="{{ route('parent.curriculum.index') }}" 
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <!-- Child and Class Info -->
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 text-gray-900">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Nama Anak</h3>
                            <p class="mt-1 text-sm text-gray-900 font-medium">{{ $child->name }}</p>
                        </div>
                        <div>
                    ...
                    @if($class->curriculum)
                        @php
                            $totalMaterials = $class->curriculum->syllabuses->sum(function($syllabus) {
                                return $syllabus->materials->count();
                            });
                            $completedCount = count($childProgress['completed']);
                            $inProgressCount = count($childProgress['in_progress']);
                            $notStartedCount = $totalMaterials - $completedCount - $inProgressCount;
                            $progressPercentage = $totalMaterials > 0 ? round(($completedCount / $totalMaterials) * 100) : 0;
                        @endphp
                        
                        <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                            <h4 class="text-sm font-medium text-gray-700 mb-3">Ringkasan Progress Pembelajaran</h4>
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                ...
                                <div class="text-center">
                                    ...
                                    <div class="text-xs text-gray-500">Sedang Dipelajari</div>
                                </div>
                                <div class="text-center">
                                    <div class="text-2xl font-bold text-gray-600">{{ $notStartedCount }}</div>
                                    <div class="text-xs text-gray-500">Belum Dimulai</div>
                                </div>
                            </div>
                                                ...
                                                <div class="flex items-center justify-between">
                                                    <div class="flex-1">
                                                        <div class="flex items-center space-x-3 mb-2">
                                                            ...
                                                            @php
                                                                $isCompleted = in_array($material->id, $childProgress['completed']);
                                                                $isInProgress = in_array($material->id, $childProgress['in_progress']);
                                                            ...
                                                            @if($isCompleted)
                                                                ...
                                                                <div>
                                                                    <span class="text-sm font-medium text-green-600">✓ Sudah Selesai Dipelajari</span>
                                                                    <p class="text-xs text-gray-500">Anak Anda telah menyelesaikan materi ini</p>
                                                                </div>
                                                            @elseif($isInProgress)
                                                                ...
                                                                <div>
                                                                    <span class="text-sm font-medium text-yellow-600">⏳ Sedang Dipelajari</span>
                                                                    <p class="text-xs text-gray-500">Anak Anda sedang mempelajari materi ini</p>
                                                                </div>
                                                            @else
                                                                ...
                                                                <div>
                                                                    <span class="text-sm font-medium text-gray-500">○ Belum Dimulai</span>
                                                                    <p class="text-xs text-gray-500">Anak Anda belum memulai materi ini</p>
                                                                </div>
                                                            @endif
                                                        </div>
                                                        
                                                        <h4 class="text-base font-medium text-gray-900 ml-11">{{ $material->title }}</h4>
                                                        @if($material->description)
                                                            <p class="text-sm text-gray-600 mt-1 ml-11">{{ $material->description }}</p>
                                                        @endif
                                                        
                                                        <div class="flex items-center space-x-4 mt-2 ml-11 text-sm text-gray-500">
