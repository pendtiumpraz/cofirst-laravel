<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('admin.materials.show', $material) }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                {{ __('Kembali') }}
            </a>
        </div>
    </x-slot>
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <select name="syllabus_id" id="syllabus_id" 
                                    ...
                                    @foreach($syllabuses as $syllabus)
                                        <option value="{{ $syllabus->id }}" {{ old('syllabus_id', $material->syllabus_id) == $syllabus->id ? 'selected' : '' }}>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" name="meeting_start" id="meeting_start" 
                                           value="{{ old('meeting_start', $material->meeting_start) }}" 
                                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                                ...
                                <div>
                                    ...
                                    <input type="number" name="meeting_end" id="meeting_end" 
                                           value="{{ old('meeting_end', $material->meeting_end) }}" 
                                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" 
                                           min="1" required placeholder="1">
                                    @error('meeting_end')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <textarea name="content" id="content" rows="8" 
                                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                          placeholder="Masukkan konten materi...">{{ old('content', $material->content) }}</textarea>
                                @error('content')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-500">Upload file baru untuk mengganti file yang ada (PDF, DOC, PPT, Video, Audio, Gambar)</p>
                                @error('file')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <select name="status" id="status" 
                                    ...
                                    <option value="inactive" {{ old('status', $material->status) == 'inactive' ? 'selected' : '' }}>Tidak Aktif</option>
                                </select>
                                @error('status')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
